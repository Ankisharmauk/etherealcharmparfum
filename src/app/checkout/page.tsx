      ...
      <section className="pt-28">
        <div className="max-w-5xl mx-auto px-6 md:px-12 py-16 grid grid-cols-1 md:grid-cols-5 gap-16">
          ...
          <div className="md:col-span-3">
            <p className="text-[#C9920E] tracking-[0.3em] uppercase text-xs mb-8 font-light" style={{ fontFamily: 'var(--font-sans)' }}>
              Your Order
            </p>
            <div className="space-y-4">
              {items.map(({ product, quantity }) => (
                <div key={product.id} className="flex items-center gap-5 border border-[#C9920E]/20 p-4 bg-[#FFFCF5]">
                  <div className="relative w-16 h-20 shrink-0 bg-[#F5E8C8] flex items-center justify-center">
                    <Image src={product.image} alt={product.name} fill className="object-contain p-2" />
                  </div>
                  <div className="flex-1">
                    <p className="text-[#2C1A06] text-sm font-light" style={{ fontFamily: 'var(--font-display)' }}>{product.name}</p>
                    <p className="text-[#2C1A06]/40 text-xs mt-1" style={{ fontFamily: 'var(--font-sans)' }}>{product.subtitle}</p>
                  </div>
          ...
          <div className="md:col-span-2">
            <div className="border border-[#C9920E]/25 bg-[#FFFCF5] p-8 sticky top-28">
              ...
              <button
                ...
                disabled={loading}
                className="w-full py-4 bg-[#C9920E] text-white text-xs tracking-[0.2em] uppercase font-medium hover:bg-[#E8A820] transition-colors disabled:opacity-50 flex items-center justify-center gap-2"
